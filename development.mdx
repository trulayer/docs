...
The OpenAPI spec is the source of truth for the HTTP API reference. It lives at `api-reference/openapi.yaml` and is a **copy** of `backend/api/openapi.yaml` from the `trulayer/backend` repo.
...
…and open a PR. CI on the backend repo will flag drift automatically (see [TRU-93](https://linear.app/omnimoda/issue/TRU-93)).
