...
1. It logs an ERROR-level message via `console.error`:
   ```
   [TruLayer] ERROR: Ingest permanently disabled — the project associated with this API key has been archived.
...
- **Create a new client** — call `new TruLayer(...)` again with the same API key. The new client is independent of the disabled one.
...
```typescript
import { TruLayerFlushError } from "@trulayer/sdk";

try {
  await tl.shutdown();
} catch (err) {
  if (err instanceof TruLayerFlushError && err.statusCode === 403) {
    console.error("Project may be archived — check app.trulayer.ai/projects");
  }
  throw err;
}
```
